...
- [Example office-holder budgets](#example-office-holder-budgets)
  - [Magistrate Hida no Reiji Kazuma of Harima County](#magistrate-hida-no-reiji-kazuma-of-harima-county)
    - [Income](#income)
    ...
    - [Kazuma's discretionary breakdown](#kazumas-discretionary-breakdown)
    - [Summary](#summary)
...
The worked examples in this document (Magistrate Kazuma's county, Governor Asuka's province, **Daimyo Isao's Reiji domain**) treat Reiji as a **hybrid "median" domain** that uses whichever multiplier produces clean worked-example numbers:
...
- The **county magistrate** at ~1,000 koku discretionary (~940 from tax surplus + ~150 from gifts in Kazuma's case, ~1,090 total) is actually wealthier than every provincial minister, despite holding a lower rank (Rank 5 vs Rank 7).  This is well-known in Rokugan and explains why ambitious junior samurai often prefer a county magistrate appointment to a provincial ministry appointment.
...
Canals are in every case a domain-and-clan matter, never a standing Imperial line.  A domain has them only where its geography offers a sizable inland river worth canalizing - the Lion's link from Toshi Ranbo to the Drowned Merchant River is a typical case - while coastal clans like the Crane, who move goods by sea and cart only the last few miles, have little need of them.  Unlike imperial China, Rokugan maintains no centrally-managed canal network: the force that made China's Grand Canal an Imperial concern was the need to haul tax grain across the empire to an inland or grain-poor northern capital, and that imperative was designed out of the Rokugani model.  Most tax grain stays in local domain granaries rather than converging on the center, and Otosan Uchi itself sits on the eastern coast and is provisioned by sea (in the manner of e.g. Rome, Alexandria, or Constantinople) rather than fed down an inland grain artery.  Any Imperial hand in canals is therefore incidental: an occasional Crown-funded major waterwork falls under the Imperial Ministry of Works' central-operations line, and any audit presence where a canal meets a city is already covered by the city-gate tariff apparatus.

Local roads tell the same story at far larger scale.  The Imperial network is deliberately sparse - ~125 miles per median domain - because its purpose is to wire the domains together into an Empire-wide trade web; within any one domain it is a through-route plus the odd spur, touching the capital and whichever few provincial cities happen to sit on the trunk.  The dense, everyday network is local: roughly ~500 miles of made road per median domain (the capital out to its ~6 provincial cities, and each of those to its ~6 county towns) plus several hundred more of village track, all built and kept up by the daimyo, provincial governors, and county magistrates.  So every provincial city is connected to the capital, but only the ones on the trunk are connected *by* Imperial road - the daimyo builds and owns the rest.  These need no separate budget line: like the Imperial roads they are corvée-built and lower-grade, so the cash cost (materials and skilled supervision) is modest and already sits inside the county, provincial, and domain Works budgets - the county-operations line already covers "minor road and bridge repairs" - while the corvée is locally levied and, unlike the Crown's Imperial-road corvée, traditionally less generously compensated, which makes a daimyo's roads cheaper to him in cash than the per-mile Imperial figure suggests.

## Example office-holder budgets

### Magistrate Hida no Reiji Kazuma of Harima County

Harima is a county in Nagahara province of the Reiji domain (Crab Clan).  Hida no Reiji Kazuma is its current magistrate, a moderately conscientious official well-regarded by peasants and respected by their retainers.  The breakdowns below show Kazuma's specific allocation choices; other magistrates with the same ~1,000-koku discretionary allocation but different personalities, lineage politics, or local circumstances allocate very differently.
...
| Heimen clerks (~3-4 × ~4-5 koku) | ~15 | Local hires from the county town (scribes by caste); they live in town and commute to the manor, and as the office's permanent staff they provide continuity across successive magistrates |
| Office servants | ~20 | Cooks, grooms, and cleaners for the compound |
| Office supplies | ~10 |  |
| **Total mandatory** |  | **~5,460** |

#### Gift income (in addition to tax revenue)

Beyond tax revenue, Magistrate Kazuma receives approximately **~150 koku per year in seasonal gifts** from local notables - primarily merchant landlords (the largest single category, since they hold most rural rentable land in the county), wealthy artisans and tradesmen (the sake brewer, the master smith, the dye-house owner), and ceremonial visitors from the magistrate's own lineage.
...
| **Subtotal: expected donations** | **~690** |  | ~63% of the allocation, consumed by never-required-but-always-expected obligations |
| Lean-year savings (forward-thinking reserve) | ~235 | Personal | ~22% put away against bad harvests; well above the irresponsible ~5-10% range that many magistrates of lesser foresight accept |
...
| **Subtotal: personal/discretionary** | **~400** |  | ~37% of allocation, of which ~60 koku is truly unconstrained |
| **Total** | **~1,090** |  |  |

#### Summary

Magistrate Hida no Reiji Kazuma is responsible for collecting ~6,400 koku in taxes annually from Harima county, of which ~5,000 goes up to Governor Hida no Reiji Asuka and ~460 covers stipends and office expenses.  The remaining ~940 koku is Kazuma's tax-derived discretionary income, supplemented by ~150 koku in seasonal gifts from local notables, for a total of ~1,090 koku per year.  After festivals, almsgiving, county operations, retainer hospitality, and the customary lineage tithe, ~400 koku is truly personal, of which Kazuma is forward-thinking enough to save ~235 against future lean years.  Kazuma's truly-unconstrained spending is around ~60 koku per year, comfortable but not lavish.

By all accounts Kazuma is a virtuous magistrate.  One in their position who skimped on festivals or pocketed the lineage donation might keep two to three times that amount for themselves, at the cost of their standing in the lineage and the community.

### Governor Hida no Reiji Asuka of Nagahara Province

Nagahara is a province in the Reiji domain (Crab Clan), containing Harima county (administered by Magistrate Kazuma, above) among its six counties.  Hida no Reiji Asuka is its current governor, a moderately accomplished official well-regarded within the Reiji Chancellery though not particularly distinguished in the broader Crab political landscape.  The breakdowns below show Asuka's specific allocation choices; other governors with the same ~10,000-koku discretionary allocation distribute very differently depending on the wealth of their province and their personal temperament.

#### Income

| Source | Annual koku | Notes |
| --- | --- | --- |
| Tax obligation from 6 county magistrates (~5,000 each) | ~30,000 | Standard provincial-tier share of county tax revenue; Kazuma's ~5,000 obligation is one of these six |
...
This totals roughly 6-7x Magistrate Kazuma's ~150-koku gift income.  The increase reflects the broader jurisdiction and richer pool of potential gifters - and the larger denominational increments scaled to the governor's higher rank - but NOT a qualitatively different scale that would mark transactional bribery.  A governor reporting 10x or more in gift income relative to a county magistrate is generally operating somewhere on the corrupt end of the spectrum.
...
| Personal household and lifestyle (clothing, sword maintenance, family expenses, political wardrobe for chancellery appearances) | ~900 | Personal | Higher than Kazuma's because the governor's household includes a wider family circle plus visible political wardrobe expectations at the chancellery |
...
**The 1st Imperial legion exception**: postings in the 1st Imperial legion (at the Gateway to the Land of the Burning Sands, traditionally commanded by the Emperor's second son) carry a **two**-rank uplift rather than the standard one - a platoon lieutenant there is Rank 7 rather than Rank 6, and the legion general is Rank 10 rather than Rank 9.  The legion's structure is also deliberately officer-heavy: smaller, more numerous platoons (4 of ~13 per company rather than 3 of ~16-20) and 6 battalions of 3-4 companies rather than the usual ~5 of ~4-5, inflating the number of officer postings available in the prince's legion.  Together these push its stipend bill to ~26,000 koku/year versus the ~16,000 typical figure below.  Part of the extra cost is offset by the Houses that provide its companies, each of which supplements its company's budget with significant extra funds so as not to be outshone by the other Houses.  See [`l7r.md` - The 1st Imperial Legion](l7r.md#the-1st-imperial-legion) for the campaign-level detail.
...
| **Total** | **~86,500** | approximated to a round number: **~85,000 Imperial appointee samurai positions Empire-wide** |

For scale: ~85,000 Imperial appointee samurai is only slightly less than the size of a mid-tier Great Clan's working samurai population.  The Phoenix and Dragon Clans have ~100,000 working samurai; the Lion, Crab, and Crane clans each substantially exceed it.  The Imperial appointee cohort is structurally a "shadow clan" without correspondingly large territorial domains, drawn from across the actual clans plus the four Imperial Families.
...
| --- | --- | --- | --- | --- |
| Senior Imperial magistrate cohort + senior Legion command (Imperial Magistrates themselves, karos, theater commanders, army generals, senior central judicial) | ~700 | ~75% (~525) | ~25% (~175) | The Seppun Family supplies the largest fraction of magistrate and karo posts.  The Hantei Family holds a small handful of the most prestigious assignments (the bulk of the Hantei Family runs Otosan Uchi's local government and the inner palace, with high-level help from the other Imperial Families; relatively few Hantei take field postings).  The Otomo and Miya Families supply some senior court roles.  Cross-clan placements - a senior samurai from one clan posted as Imperial magistrate in another - cover the residual ~25% |
| Mid-tier Imperial posts (mid-Legion officers, waystation relay-masters, mid-tier central roles, and samurai running the personal households of Imperial magistrates) | ~10,000 | ~8% (~800) | ~92% (~9,200) | Imperial Families concentrate their mid-tier share in posts that touch high-prestige central work (Imperial magistrate personal household for senior magistrates, top Legion staff officers, Otosan Uchi government leadership).  Most mid-tier posts come from the clans |
...
| Imperial yoriki and household samurai at other domains' main offices | ~27 | Cross-clan tours in office yoriki roles, working under senior IMs (mostly Seppun or cross-clan senior placements); at the senior tier, Reiji's contribution is reduced (~25% of senior IM/karo posts are clan-supplied, with the bulk going to Seppun and the other Imperial Families) |
| Imperial yoriki at provincial sub-stations in other domains | ~27 | Cross-clan rotating tours; a Reiji samurai might serve 3-5 years auditing tariff collection in (say) a Lion province before returning home |
| Imperial appointees at waystations in other domains (mix of relay-masters and waystation yoriki) | ~26 | Spread across waystation roles in non-Reiji parts of the Empire |
| Central Imperial roles in Otosan Uchi (OU local government, central court samurai, central judicial machinery, central ministries other than the Imperial magistrate cohort field roles) | ~50 | Imperial-paid central posts physically located at the capital.  These ~50 are the Imperial-post component of Reiji's ~100-strong total Otosan Uchi presence (the rest being the ~38 clan-paid embassy and ~12 private samurai - see [Samurai concentration by city size and the capital](#samurai-concentration-by-city-size)).  Central roles favor the Imperial Families more heavily, but the absolute clan share is still substantial |
| Other Imperial ministries' field operations (Works engineering, Rites ceremonial, Revenue inspection, etc) | ~3 | Specialized assignments matched to a samurai's particular skills (e.g. an unusually skilled logistics officer posted to Wall-construction stonemason supervision under the Imperial Ministry of Works) |
...
The Empire's current state (per the campaign-current understanding) is that recent years have been very hard on Imperial finances, and the practical pool of total Imperial reserves (vault savings and callable obligations combined) is down to roughly 10 million koku - under a third of one year's revenue, and roughly an order of magnitude below the well-run-empire norm of ~2-3 years.  By the standards of any well-administered premodern empire this is a genuine fiscal crisis, and the Imperial Treasurer is under considerable pressure to rebuild reserves.  This number can shift with events of ongoing campaigns.

The proximate cause of the current crisis is well-known in the Imperial Court.  The passing of Hantei the XXXVIII's mother brought an unusually long mourning period with especially generous Empire-wide grain distributions, reflecting the depth of the Emperor's feeling for her; at the time of her death the reserves were only slightly below normal, but the extended observances consumed several years' worth of accumulated savings.  This drawdown was followed almost immediately by a massive harbor renovation in Otosan Uchi, after which the Emperor spend considerable resources preventing a war between the Crab and Scorpion clans.  Unfortunately, the **Lion/Crane war** broke out shortly after this, followed by the **Lion/Unicorn war** that erupted in the final 3 years of the Lion/Crane war and was settled alongside it.  The Imperial treasury bore years of elevated military costs described in the legions section above (field-rate deployments, freshly raised legions, full Imperial funding throughout), on top of magistrate interventions, supply disruption to the Wall, and emergency famine relief in war-affected counties.  Between the mourning and the wars the reserves fell from near-normal to the current crisis level, and the Emperor has been raising Otosan Uchi tariffs incrementally (now ~15% rather than the 5% baseline) to halt the bleeding - so far without enough success to begin rebuilding.

The ongoing cost of those deployments is also what made it *rational* for the Emperor to fund many of the peace terms personally, out of the Imperial treasury.  The largest single such expenditure was a personal pledge of **5 million koku to Moto Gaheris** - extraordinary not only in size but in being paid in coin, gems, silk, and other valuables rather than grain, which makes its true cost far higher than the raw figure suggests.  This was far from the only such "gift"; the Hantei quietly absorbed a long list of lesser treaty provisions as well, though by no means all of them.  To be clear, the Emperor paid from a position of *strength*, not weakness.  His goal was to end both wars without taking a military side - which would have alienated a Great Clan and upset the thousand-year balance - without letting the fighting grind on at ruinous expense.  Much of his work at the negotiating table was the patient demonstration that, to reach a settlement, he could if he chose impose far less favorable terms on any party, and that each of them stood to lose a great deal if he did.

That leverage was concrete.  The **Crane** carried the original fault; it was the Tsume vassal house of the Doji Family that set the disorder in motion by invading Damasu lands and seizing Toshi Ranbo, and both the Tsume and the Crane at large could have been punished far more harshly for it.  The **Lion** finished the Lion/Crane conflict as nominal victors (holding more land than they began with), but because every Lion samurai swears *two* blood oaths, one to their daimyo and one to the Emperor, Hantei the XXXVIII could have required the Lion to pay for Imperial recognition of those gains, compelling even low-ranking Lion samurai to help enforce the demand on pain of breaking the very oaths their daimyo's authority depends upon.  The **Unicorn** were militarily ascendant but politically brittle; Shinjo Yokatsu had been deposed for religious heresy, and his successor Shinjo Hanari had held the Championship only a few years before the Lion invasion (which is partially why Hanari owes his survival to Moto Gaheris rather than to his own forces), so the Emperor could have demanded a far larger Unicorn contribution toward appeasing Gaheris and settling with the Lion, under the implicit threat of recognizing some other lineage, house, or Family as the clan's rightful rulers.

In the event, the Emperor and his advisors did extract genuine concessions from each party, and pressed on every signatory some small, pointed term so the lesson could not be missed.  Still, the Hantei nonetheless paid for far more of the settlement than they were strictly obliged to.  That was the deliberate strategy, not weak bargaining: a measured show of strength (an unwelcome condition or two for everyone, proof the throne could compel) wrapped in conspicuous generosity (the throne quietly absorbing the costs), so that each clan came away both reminded of Imperial power and glad to keep participating in the Imperial system as it stands.  The price has been steep - the treasury is now lower than it has been in centuries, low enough to drive the Emperor to the loans and bargains described below with terms he would never otherwise entertain - but it is an understandable trade, and entirely of a piece with the strategy and ethos by which the Hantei have held control of the Empire for over a thousand years.

Properly understood, then, the depleted treasury is the result of **prudence and long-term thinking, not financial mismanagement**.  In a normal year the Empire runs a healthy surplus (~4-5M against ~34-36M of revenue) and builds its reserves steadily; the current shortfall is the mark of a dynasty that chose to absorb a generational run of shocks in full rather than appear weak or take sides.  There were, to be fair, some avoidable outlays in the years before the crises, which had already drawn the treasury down to perhaps 60-80% of its usual reserve level, but with no warning of the hard years ahead, that was a reasonable posture rather than a careless one.  (Astute readers may infer, correctly, that the current generation of prophets in the Order of Amaterasu includes no servant of Daikoku, the Fortune of Wealth, of the kind who once let Hantei the Tenth foresee the long-run consequences of fiscal decisions; had one been at court, the Emperor might have been counseled to hoard his reserves against the coming years.)  Even a healthy surplus, however, cannot refill a hole this deep in any reasonable span (rebuilding from the current ~10M toward the usual 75-100M would take well over a decade of ordinary saving) so the Hantei have also resorted to measures well outside the ordinary fiscal toolkit.  This includes:
...
>
> Hantei the X considered this, and then named a reduction in budgets more significant than what Taka had proposed, asking what size his treasury would be had he established this reduction at the beginning of his reign.
>
...
#### Why gifts to the throne aren't a revenue line

A reader totting up Imperial revenue might expect gifts *to* the Emperor - the upward counterpart of all this Imperial generosity - to form a line of their own.  They do not, and the Yasuki Taka story is exactly why: the throne's gift economy is a **callable reserve, not annual income**.  The large sums materialize when the Emperor *asks*, drawn down from the favor-bank the Crown has spent reigns building through its own largesse - reserve wealth already counted above, and lumpy and event-driven (a coronation, a mourning, a sudden Imperial need) rather than something that arrives every fiscal year.

Three things keep the rest off the revenue ledger:
* **Reciprocity**: routine court gifts run both directions, and a throne whose whole strategy is conspicuous magnanimity gives back at least as much as it takes in - in the Chinese tribute model the ruler's return-gifts often *exceeded* the tribute, since out-giving one's subordinates was the entire point - so in a normal year the gift economy nets to roughly zero, or even a prestige expense, rather than a surplus.
* **Fungibility**: much of what is gifted is prestige goods - fine weapons, art, horses, ceremonial treasures - which adorn the court, get regifted, or rest in the treasury of curiosities, but cannot pay a legionnaire's stipend or pave a mile of road.
* The one historical channel that *did* turn gifts into real revenue - the sale of office for a "gift," as in the Mughal and Ottoman *pishkash* - is designed out of Rokugan, where Imperial posts are filled by examination and chancellery selection rather than bought. 

What remains as genuinely fungible, genuinely annual gift income is small enough to fold into the Emperor's discretionary spending, and is treated that way rather than broken out.
...
- **~4% is suitable for rice farming**: approximately 60,000 square miles total, or ~150 square miles per median-sized domain.  This 4% figure follows imperial Chinese norms (historically ~3-5% of total area, depending on period); Tokugawa Japan's higher ~6% paddy share is not the right anchor for Rokugan, whose China-scale continental geography includes mountain ranges, arid borderlands, and frontier regions without close Japanese analogues.
...
**County and domain family counts**: a  median county has `156 families (town) + 6 * 70 families per village + 36 * 15 families per hamlet = 1,116 farming families`.  A median domain has `36 * 1,116 = 40,176` farming families.

**Average koku per family**: a median domain has ~250,000 population, and assuming ~2 koku worth of food per person, that's `500,000 koku per year / 40,176 families ~= 12.45 koku per family`.  This rounds up to ~15 koku per family on average, reflecting a typical surplus.
